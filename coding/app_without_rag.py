...
st.markdown(
    """Welcome to our bicycle recommendation service! Let's find the perfect bike based on your preferences. Please provide as much detail as possible so I can suggest the best options for you.""")

# Add a switch to enable/disable RAG
use_rag = st.checkbox("Enable Retrieval-Augmented Generation (RAG)", value=True)
...
if question := st.chat_input("What's up?"):
    ...
    if use_rag:
        # Generate the answer by calling OpenAI's Chat Model with RAG
        inputs = RunnableMap({
            'context': lambda x: retriever.invoke(x['question']),
            'question': lambda x: x['question']
        })
    else:
        # Generate the answer by calling OpenAI's Chat Model without RAG
        inputs = RunnableMap({
            'context': lambda x: "",
            'question': lambda x: x['question']
        })
